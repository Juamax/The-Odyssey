...
class MiniJuego1:
    ...
    def jugar(self):
        ...
        try:
            frames_enemigo = self.cargar_gif("Minijuego_Poseidon/poseidon.gif", escala=(180, 180))  # reducido para dar más espacio de la resolucionn
            if not frames_enemigo:
                raise ValueError("El GIF del enemigo no tiene frames")

            frames_jugador = self.cargar_gif("Minijuego_Poseidon/odiseo.gif", escala=(90, 90))
            if not frames_jugador:
                raise ValueError("El GIF del jugador no tiene frames")

            fondo_img = pygame.image.load("Minijuego_Poseidon/oceano.jpg").convert()
            fondo_img = pygame.transform.scale(fondo_img, (self.ancho, self.alto))

            img_lluvia = pygame.image.load("Minijuego_Poseidon/agua.png").convert_alpha()
            img_lluvia = pygame.transform.scale(img_lluvia, (18, 40))

            img_diagonal = pygame.image.load("Minijuego_Poseidon/tridente.png").convert_alpha()
            img_diagonal = pygame.transform.scale(img_diagonal, (24, 24))

            img_lateral = pygame.image.load("Minijuego_Poseidon/agua.png").convert_alpha()
