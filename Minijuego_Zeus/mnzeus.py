...
class MiniJuego1:
    ...
    def jugar(self):
        ...
        try:
            ...
            frames_enemigo = self.cargar_gif("Minijuego_Zeus/zeuspelea.gif", escala=(120, 120))
            if not frames_enemigo:
                raise ValueError("El GIF del enemigo no tiene frames")

            frames_jugador = self.cargar_gif("Minijuego_Zeus/Athenapelea.gif", escala=(100, 100))
            if not frames_jugador:
                raise ValueError("El GIF del jugador no tiene frames")

            fondo_img = pygame.image.load("Minijuego_Zeus/fondo.jpg").convert()
            fondo_img = pygame.transform.scale(fondo_img, (self.ancho, self.alto))

            img_lluvia = pygame.image.load("Minijuego_Zeus/rayodos.png").convert_alpha()
            img_lluvia = pygame.transform.scale(img_lluvia, (18, 40))

            img_diagonal = pygame.image.load("Minijuego_Zeus/nube.png").convert_alpha()
            img_diagonal = pygame.transform.scale(img_diagonal, (24, 24))

            img_lateral = pygame.image.load("Minijuego_Zeus/rayodos.png").convert_alpha()
