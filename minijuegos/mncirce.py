...
class MiniJuego1:
    ...
    def jugar(self):
        ...
        try:
            frames_enemigo = self.cargar_gif("minijuegos/circe.gif", escala=(180, 180))  # era 200x200
            if not frames_enemigo:
                raise ValueError("El GIF del enemigo no tiene frames")
            frames_jugador = self.cargar_gif("minijuegos/odiseo.gif", escala=(100, 100))
            if not frames_jugador:
                raise ValueError("El GIF del jugador no tiene frames")

            fondo_img = pygame.image.load("minijuegos/fondo.png").convert()
            fondo_img = pygame.transform.scale(fondo_img, (self.ancho, self.alto))

            img_lluvia = pygame.image.load("minijuegos/bola.png").convert_alpha()
            img_lluvia = pygame.transform.scale(img_lluvia, (18, 40))
            img_diagonal = pygame.image.load("minijuegos/bola.png").convert_alpha()
            img_diagonal = pygame.transform.scale(img_diagonal, (24, 24))
            img_lateral = pygame.image.load("minijuegos/bola.png").convert_alpha()
